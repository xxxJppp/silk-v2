...
public class ArchiveTableCreatorJob extends AbstractTableCreator {
    ...
    private void createWalRecordHistory() {
        String table = "exchange_wallet_wal_record_his_" + getDatetimeStringOfTomorrow();
        String sql = "CREATE TABLE `%s` like exchange_wallet_wal_record";
        doCreate(table, sql);
    }
    ...
    private void createExchangeCywOrderHistory() {
        String table = "exchange_order_his_" + getCurrentMonth();
        String sql = "CREATE TABLE `%s` like exchange_order";
        doCreate(table, sql);
    }
