...
@Mapper
@Repository
public interface ExchangeWalletWalExtMapper {
    ...
    @Select("SELECT id, ref_id, trade_type, trade_balance,trade_frozen FROM exchange_wallet_wal_record r WHERE r.status = 1 AND r.ref_id = #{refId}")
    ...
    @Delete("delete from exchange_wallet_wal_record where ref_id = #{refId}")
     ...
     * @return int
     */
    int saveBatch(@Param("table") String table, @Param("records") List<ExchangeWalletWalRecord> records);
}
